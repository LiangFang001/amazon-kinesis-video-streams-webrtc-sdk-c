    ...
    if (pSampleConfiguration->useTurn) {
        ...
        for (uriCount = 0, i = 0; i < maxTurnServer; i++) {
            ...
            for (j = 0; j < pIceConfigInfo->uriCount; j++) {
                 ...
                 * over DTLS yet. if pConfiguration->iceServers[uriCount + 1].urls is "turns:ip:port?transport=tcp" then ICE will try TURN over
                 * TCP/TLS if pConfiguration->iceServers[uriCount + 1].urls is "turn:ip:port" then ICE will try both TURN over UPD and TCP/TLS
                 *
