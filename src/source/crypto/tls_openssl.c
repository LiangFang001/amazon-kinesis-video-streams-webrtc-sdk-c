...
INT32 tls_session_certificateVerifyCallback(INT32 preverify_ok, X509_STORE_CTX* ctx)
{
...
STATUS tls_session_start(PTlsSession pTlsSession, BOOL isServer)
{
    ...
    SSL_CTX_set_verify(pTlsSession->pSslCtx, SSL_VERIFY_PEER | SSL_VERIFY_FAIL_IF_NO_PEER_CERT, tls_session_certificateVerifyCallback);
    ...
    /* send handshake */
    CHK_STATUS(tls_session_send(pTlsSession, NULL, 0));
        ...
        if (sslRet < 0) {
            ...
            switch (sslErr) {
                ...
                default:
                    ...
                    tls_session_changeState(pTlsSession, TLS_SESSION_STATE_CLOSED);
                    break;
            }
