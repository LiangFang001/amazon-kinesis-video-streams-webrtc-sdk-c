...
// check if pIceCandidatePairs is in descending order
BOOL candidatePairsInOrder(PDoubleList pIceCandidatePairs)
{
    BOOL inOrder = TRUE;
...
TEST_F(IceFunctionalityTest, sortIceCandidatePairsTest)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, ice_candidate_pair_insert(iceAgent.pIceCandidatePairs, &iceCandidatePair[0]));
    EXPECT_EQ(TRUE, candidatePairsInOrder(iceAgent.pIceCandidatePairs));
    EXPECT_EQ(STATUS_SUCCESS, doubleListClear(iceAgent.pIceCandidatePairs, FALSE));

    iceCandidatePair[0].priority = 1;
    iceCandidatePair[1].priority = 2;
    for (i = 0; i < 2; ++i) {
        EXPECT_EQ(STATUS_SUCCESS, ice_candidate_pair_insert(iceAgent.pIceCandidatePairs, &iceCandidatePair[i]));
    }
    EXPECT_EQ(TRUE, candidatePairsInOrder(iceAgent.pIceCandidatePairs));
    EXPECT_EQ(STATUS_SUCCESS, doubleListClear(iceAgent.pIceCandidatePairs, FALSE));
    ...
    EXPECT_EQ(TRUE, candidatePairsInOrder(iceAgent.pIceCandidatePairs));
    EXPECT_EQ(STATUS_SUCCESS, doubleListClear(iceAgent.pIceCandidatePairs, FALSE));
    EXPECT_EQ(STATUS_SUCCESS, doubleListFree(iceAgent.pIceCandidatePairs));
}
...
TEST_F(IceFunctionalityTest, IceAgentIceAgentAddIceServerUnitTest)
{
    ...
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "stun:stun.kinesisvideo.us-west-2.amazonaws.com:443", NULL, NULL));
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "stun:stun.kinesisvideo.us-west-2.amazonaws.com:443", (PCHAR) "", (PCHAR) ""));

    EXPECT_NE(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, NULL, NULL, NULL));
    EXPECT_EQ(STATUS_ICE_UTILS_URL_TURN_MISSING_USERNAME, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443", NULL, NULL));
    EXPECT_EQ(STATUS_ICE_UTILS_URL_TURN_MISSING_CREDENTIAL, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443", (PCHAR) "username", NULL));
    EXPECT_EQ(STATUS_ICE_UTILS_URL_TURN_MISSING_USERNAME, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443", (PCHAR) "", (PCHAR) ""));
    EXPECT_EQ(STATUS_ICE_UTILS_URL_TURN_MISSING_CREDENTIAL, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443", (PCHAR) "username", (PCHAR) ""));
    EXPECT_NE(STATUS_SUCCESS, ice_utils_parseIceServer(NULL, (PCHAR) "turn:54.202.170.151:443", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turns:54.202.170.151:443", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_TRUE(iceServer.isSecure);
    EXPECT_EQ(iceServer.transport, KVS_SOCKET_PROTOCOL_NONE);
    EXPECT_EQ(STATUS_ICE_UTILS_URL_INVALID_PREFIX, ice_utils_parseIceServer(&iceServer, (PCHAR) "randomUrl", (PCHAR) "username", (PCHAR) "password"));

    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turns:54.202.170.151:443?transport=tcp", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_TRUE(iceServer.isSecure);
    EXPECT_EQ(iceServer.transport, KVS_SOCKET_PROTOCOL_TCP);
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turns:54.202.170.151:443?transport=udp", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_TRUE(iceServer.isSecure);
    EXPECT_EQ(iceServer.transport, KVS_SOCKET_PROTOCOL_UDP);
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443?transport=tcp", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_TRUE(!iceServer.isSecure);
    EXPECT_EQ(iceServer.transport, KVS_SOCKET_PROTOCOL_TCP);
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443?transport=udp", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_TRUE(!iceServer.isSecure);
    EXPECT_EQ(iceServer.transport, KVS_SOCKET_PROTOCOL_UDP);
    EXPECT_EQ(443, (UINT16) getInt16(iceServer.ipAddress.port));

    /* we are not doing full validation. Only parsing out what we know */
    EXPECT_EQ(STATUS_SUCCESS, ice_utils_parseIceServer(&iceServer, (PCHAR) "turn:54.202.170.151:443?randomstuff", (PCHAR) "username", (PCHAR) "password"));
    EXPECT_EQ(iceServer.transport, KVS_SOCKET_PROTOCOL_NONE);
}
...
TEST_F(IceFunctionalityTest, IceAgentPruneUnconnectedIceCandidatePairUnitTest)
{
    ...
    PIceCandidatePair pIceCandidatePairs[5];
    PDoubleListNode pCurNode = NULL;
    PIceCandidatePair pIceCandidatePair = NULL;
    ...
    EXPECT_EQ(STATUS_SUCCESS, ice_candidate_pair_pruneUnconnected(&iceAgent));

    for (i = 0; i < 5; ++i) {
        pIceCandidatePairs[i] = (PIceCandidatePair) MEMCALLOC(1, SIZEOF(IceCandidatePair));
        pIceCandidatePairs[i]->priority = i * 100;
        pIceCandidatePairs[i]->state = (ICE_CANDIDATE_PAIR_STATE) i;
        EXPECT_EQ(STATUS_SUCCESS, transaction_id_store_create(DEFAULT_MAX_STORED_TRANSACTION_ID_COUNT, &pIceCandidatePairs[i]->pTransactionIdStore));
        EXPECT_EQ(STATUS_SUCCESS, ice_candidate_pair_insert(iceAgent.pIceCandidatePairs, pIceCandidatePairs[i]));
    }
